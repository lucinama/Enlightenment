...
EOAPI EO_VOID_FUNC_BODYV(simple_xy_set, EO_FUNC_CALL(x, y), int x, int y);

void _simple_xy_get(Eo *obj, Simple_Data *pd, int *x, int *y);

EOAPI EO_VOID_FUNC_BODYV(simple_xy_get, EO_FUNC_CALL(x, y), int *x, int *y);

int _simple_mxy_get(Eo *obj, Simple_Data *pd);

EOAPI EO_FUNC_BODY(simple_mxy_get, int, 0);
